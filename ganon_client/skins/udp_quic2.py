...
class _GanonQuic2Protocol(QuicConnectionProtocol):
    ALPN = "ganon-q2"
    ...
    def send_frame(self, plaintext: bytes) -> None:
        if self._stream_id is None or self._closed:
            raise ConnectionError("QUIC2 connection not ready")
...
class UdpQuic2Skin(NetworkSkinImpl):
    """QUIC transport skin using ngtcp2-compatible ALPN 'ganon-q2'."""

    def __init__(self, protocol: _GanonQuic2Protocol, exit_stack):
        self._proto = protocol
        self._exit_stack = exit_stack

    @classmethod
    async def open(cls, ip: str, port: int, timeout: float) -> "UdpQuic2Skin":
        config = QuicConfiguration(
            alpn_protocols=[_GanonQuic2Protocol.ALPN],
            is_client=True,
        ...
        try:
            ctx = quic_connect(
                ...
                create_protocol=_GanonQuic2Protocol,
            )
            ...
            raise ConnectionError(
                f"QUIC2 connect to {ip}:{port} timed out after {timeout}s"
            )
        except Exception as e:
            await exit_stack.aclose()
            raise ConnectionError(
                f"QUIC2 connect to {ip}:{port} failed: {e}"
            ) from e
    ...
    async def send(self, plaintext: bytes) -> None:
        try:
            ...
            raise ConnectionError(f"QUIC2 send failed: {e}") from e
...
register_skin(NetworkSkin.UDP_QUIC2, UdpQuic2Skin)
