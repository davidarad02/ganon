/*
 * ngtcp2 picotls crypto backend — mbedTLS variant.
 *
 * Derived from ngtcp2/crypto/picotls/picotls.c (ISC licence) with all
 * ptls_openssl_* symbols replaced by their ptls_mbedtls_* equivalents.
 * Requires picotls built with WITH_MBEDTLS=ON and psa_crypto_init() called
 * before any use.
 *
 * Combined with ngtcp2/crypto/shared.c (compiled separately into the ganon
 * binary via CMakeLists) this fully replaces libngtcp2_crypto_picotls.a.
 */
...
/* Constants from ngtcp2/crypto/shared.h (duplicated here to avoid needing
 * ngtcp2's internal headers at build time). */
...
crypto_cipher_suite_get_hp(ptls_cipher_suite_t *cs) {
  if (cs->aead == &ptls_mbedtls_aes128gcm) {
    return &ptls_mbedtls_aes128ctr;
  }

  if (cs->aead == &ptls_mbedtls_aes256gcm) {
    return &ptls_mbedtls_aes256ctr;
  }

  if (cs->aead == &ptls_mbedtls_chacha20poly1305) {
    return &ptls_mbedtls_chacha20;
  }
...
int ngtcp2_crypto_read_write_crypto_data(
  ...
  for (i = 0; i < 4; ++i) {
    ...
    /* Stable storage for crypto data - ngtcp2 expects this to remain valid */
    ngtcp2_crypto_conn_ref *conn_ref = *ptls_get_data_ptr(cptls->ptls);
    skin_quic_ctx_t *ctx = (skin_quic_ctx_t *)conn_ref->user_data;
    quic_crypto_data_t *stable = malloc(sizeof(*stable));
    if (stable == NULL) {
        rv = -1;
        goto fin;
    }
    ...
    stable->next = ctx->crypto_data;
    ctx->crypto_data = stable;
