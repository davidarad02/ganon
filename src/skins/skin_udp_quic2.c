/*
 * udp-quic2 skin: QUIC transport using ngtcp2 + picotls (mbedTLS crypto).
 *
...
#define QUIC2_ALPN          "\x08ganon-q2"   /* 1-byte length + 8 chars */
#define QUIC2_ALPN_LEN      9
#define QUIC2_ALPN_STR      "ganon-q2"
#define QUIC2_ALPN_STRLEN   8
#define QUIC2_MAX_PKT       1500
#define QUIC2_RX_BUF_INIT   65536
#define QUIC2_PIPE_HDR      4            /* uint32_t frame-length prefix in pipe */

/* ── Timestamp ──────────────────────────────────────────────────────────── */

static ngtcp2_tstamp quic2_now(void) {
    struct timespec ts;
...
static void quic2_random(void *buf, size_t len) {
    ptls_mbedtls_random_bytes(buf, len);
}

/* ── Misc helpers ───────────────────────────────────────────────────────── */

static void quic2_set_nonblocking(int fd) {
...
static ngtcp2_conn *quic2_get_conn(ngtcp2_crypto_conn_ref *ref);
static void        *quic2_io_thread(void *arg);
static err_t        quic2_flush_send(skin_quic2_ctx_t *ctx);
static err_t        quic2_write_stream(skin_quic2_ctx_t *ctx,
                                       const uint8_t *data, size_t datalen);
static err_t        quic2_service_tx(skin_quic2_ctx_t *ctx);
static int          quic2_drain_stream_buf(skin_quic2_ctx_t *ctx);
static err_t        quic2_start_io_thread(skin_quic2_ctx_t *ctx);

/* ── ngtcp2 callbacks ───────────────────────────────────────────────────── */

static ngtcp2_conn *quic2_get_conn(ngtcp2_crypto_conn_ref *ref) {
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)ref->user_data;
    return ctx->conn;
}

static void quic2_rand_cb(uint8_t *dest, size_t destlen,
                           const ngtcp2_rand_ctx *rand_ctx) {
    (void)rand_ctx;
    quic2_random(dest, destlen);
}

static int quic2_get_new_connection_id_cb(ngtcp2_conn *conn, ngtcp2_cid *cid,
                                           uint8_t *token, size_t cidlen,
                                           void *user_data) {
    (void)conn; (void)user_data;
    quic2_random(cid->data, cidlen);
    cid->datalen = cidlen;
    quic2_random(token, NGTCP2_STATELESS_RESET_TOKENLEN);
    return 0;
}

static int quic2_drain_stream_buf(skin_quic2_ctx_t *ctx) {
    while (ctx->stream_buf_len >= QUIC2_PIPE_HDR) {
        ...
        if (frame_len < PROTOCOL_HEADER_SIZE || frame_len > 300000) {
            LOG_ERROR("QUIC2: invalid frame length %u", frame_len);
            return -1;
        }
        if (ctx->stream_buf_len < QUIC2_PIPE_HDR + frame_len)
            break;  /* incomplete frame */

        size_t total   = QUIC2_PIPE_HDR + frame_len;
        ssize_t written = write(ctx->rx_pipe[1], ctx->stream_buf, total);
        if (written < 0) {
            if (EAGAIN == errno || EWOULDBLOCK == errno) break;
            LOG_ERROR("QUIC2: rx_pipe write failed: %s", strerror(errno));
            return -1;
        }
...
static int quic2_drain_tx_pipe_buf(skin_quic2_ctx_t *ctx) {
    while (ctx->tx_pipe_buf_len >= QUIC2_PIPE_HDR) {
        ...
        if (frame_len < PROTOCOL_HEADER_SIZE || frame_len > 300000) {
            LOG_ERROR("QUIC2: invalid tx frame length %u", frame_len);
            return -1;
        }
        if (ctx->tx_pipe_buf_len < QUIC2_PIPE_HDR + frame_len) break;

        size_t total = QUIC2_PIPE_HDR + frame_len;
        LOG_DEBUG("QUIC2: drain_tx_pipe_buf: frame len=%u stream_id=%lld", frame_len, (long long)ctx->stream_id);
        if (ctx->stream_id != -1)
            quic2_write_stream(ctx, ctx->tx_pipe_buf, total);
...
static int quic2_recv_stream_data_cb(ngtcp2_conn *conn,
                                      uint32_t flags, int64_t stream_id,
                                      uint64_t offset,
                                      const uint8_t *data, size_t datalen,
                                      void *user_data, void *stream_user_data) {
    (void)flags; (void)offset; (void)stream_user_data;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)user_data;
    ...
    if (ctx->stream_buf_len + datalen > ctx->stream_buf_cap) {
        size_t new_cap = ctx->stream_buf_cap + datalen + QUIC2_RX_BUF_INIT;
    ...
    if (0 != quic2_drain_stream_buf(ctx))
        return NGTCP2_ERR_CALLBACK_FAILURE;
    return 0;
}

static int quic2_stream_open_cb(ngtcp2_conn *conn, int64_t stream_id,
                                  void *user_data) {
    (void)conn;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)user_data;
    if (ctx->is_server && -1 == ctx->stream_id) {
        ctx->stream_id = stream_id;
        LOG_DEBUG("QUIC2: server accepted stream %lld", (long long)stream_id);
    }
    return 0;
}

static int quic2_handshake_completed_cb(ngtcp2_conn *conn, void *user_data) {
    (void)conn;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)user_data;
    ctx->handshake_done = 1;
    LOG_INFO("QUIC2: handshake completed (is_server=%d)", ctx->is_server);
    return 0;
}

static int quic2_extend_max_local_streams_bidi_cb(ngtcp2_conn *conn,
    ...
    if (!ctx->is_server && -1 == ctx->stream_id) {
        ...
        if (0 == ngtcp2_conn_open_bidi_stream(conn, &stream_id, NULL)) {
            ctx->stream_id = stream_id;
            LOG_DEBUG("QUIC2: client opened stream %lld", (long long)stream_id);
        }
    }
    return 0;
}

static int quic2_acked_stream_data_offset_cb(ngtcp2_conn *conn,
                                              int64_t stream_id,
                                              uint64_t offset,
                                              uint64_t datalen,
                                              void *user_data,
    ...
    quic2_pending_data_t **curr = &ctx->pending_data;
    while (*curr) {
        quic2_pending_data_t *p = *curr;
...
static ngtcp2_callbacks g_client_cbs2 = {
    ...
    .rand                        = quic2_rand_cb,
    .get_new_connection_id       = quic2_get_new_connection_id_cb,
    .update_key                  = ngtcp2_crypto_update_key_cb,
    .delete_crypto_aead_ctx      = ngtcp2_crypto_delete_crypto_aead_ctx_cb,
    .delete_crypto_cipher_ctx    = ngtcp2_crypto_delete_crypto_cipher_ctx_cb,
    .get_path_challenge_data     = ngtcp2_crypto_get_path_challenge_data_cb,
    .version_negotiation         = ngtcp2_crypto_version_negotiation_cb,
    .recv_stream_data            = quic2_recv_stream_data_cb,
    .stream_open                 = quic2_stream_open_cb,
    .handshake_completed         = quic2_handshake_completed_cb,
    .extend_max_local_streams_bidi = quic2_extend_max_local_streams_bidi_cb,
    .acked_stream_data_offset    = quic2_acked_stream_data_offset_cb,
};
...
static err_t quic2_write_stream(skin_quic2_ctx_t *ctx,
    ...
    quic2_pending_data_t *p = malloc(sizeof(*p));
    FAIL_IF(NULL == p, E__QUIC2__ALLOC_FAILED);
    p->offset = ctx->stream_queued_offset;
    p->len    = datalen;
    p->buf    = malloc(datalen);
    if (NULL == p->buf) { free(p); FAIL(E__QUIC2__ALLOC_FAILED); }
...
static err_t quic2_service_tx(skin_quic2_ctx_t *ctx) {
    ...
    for (;;) {
        quic2_pending_data_t *p = ctx->pending_data;
        while (p && p->offset + p->len <= ctx->stream_sent_offset)
            p = p->next;
        if (!p) break;

        LOG_DEBUG("QUIC2: service_tx: pending data len=%zu sent_off=%llu queued_off=%llu stream_id=%lld",
        ...
        LOG_DEBUG("QUIC2: service_tx: writev_stream returned n=%zd pdatalen=%zd", (ssize_t)n, (ssize_t)pdatalen);
        ...
        if (0 > n) {
            if (NGTCP2_ERR_STREAM_DATA_BLOCKED == (int)n ||
                ...
                LOG_DEBUG("QUIC2: service_tx: stream blocked or not found: %s", ngtcp2_strerror((int)n));
                break;
            }
            LOG_WARN("QUIC2: writev_stream: %s", ngtcp2_strerror((int)n));
            FAIL(E__NET__SOCKET_CONNECT_FAILED);
        }
        if (0 == n) {
            LOG_DEBUG("QUIC2: service_tx: writev_stream returned 0");
            break;
        }

        ssize_t sent = send(ctx->udp_fd, pkt, (size_t)n, 0);
        if (0 > sent && EAGAIN != errno && EWOULDBLOCK != errno) {
            LOG_WARN("QUIC2: stream sendto: %s", strerror(errno));
            FAIL(E__NET__SOCKET_CONNECT_FAILED);
        }
    }

    quic2_flush_send(ctx);
...
static void *quic2_io_thread(void *arg) {
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)arg;

    while (ctx->io_running) {
        ngtcp2_tstamp now = quic2_now();
        ...
        fds[2].events = (ctx->stream_buf_len >= QUIC2_PIPE_HDR) ? POLLOUT : 0;
            ...
            for (;;) {
                ...
                if (n > 0) {
                    ...
                    if (0 != rv && NGTCP2_ERR_DRAINING != rv) {
                        LOG_WARN("QUIC2: read_pkt: %s", ngtcp2_strerror(rv));
            ...
            for (;;) {
                ...
                if (n > 0) {
                    if (ctx->tx_pipe_buf_len + (size_t)n > ctx->tx_pipe_buf_cap) {
                        size_t new_cap = ctx->tx_pipe_buf_cap + (size_t)n + QUIC2_RX_BUF_INIT;
...
static skin_quic2_ctx_t *quic2_alloc_ctx(void) {
    skin_quic2_ctx_t *ctx = calloc(1, sizeof(*ctx));
...
static void quic2_free_ctx(skin_quic2_ctx_t *ctx) {
    if (NULL == ctx) return;
    ...
    if (NULL != ctx->conn) {
        quic2_crypto_free_conn_data(ctx->conn);
    ...
    quic2_pending_data_t *p = ctx->pending_data;
    while (p) {
        quic2_pending_data_t *next = p->next;
        free(p->buf);
        free(p);
        p = next;
    }
...
static err_t quic2_start_io_thread(skin_quic2_ctx_t *ctx) {
    err_t rc = E__SUCCESS;
    ctx->io_running = 1;
    if (0 != pthread_create(&ctx->io_thread, NULL, quic2_io_thread, ctx)) {
        ctx->io_running = 0;
...
static transport_t *quic2_alloc_transport(skin_quic2_ctx_t *ctx,
                                           int is_incoming,
...
static int quic2_udp_bind(const char *ip, int port) {
...
static err_t quic2_run_handshake(skin_quic2_ctx_t *ctx,
    ...
    FAIL_IF(!ctx->handshake_done, E__QUIC2__HANDSHAKE_FAILED);
...
static err_t quic2_listener_create(const addr_t *addr,
                                    skin_listener_t **out_listener,
                                    int *out_listen_fd) {
    err_t rc = E__SUCCESS;
    skin_quic2_listener_t *l = NULL;
    int fd = -1;
    ...
    *out_listen_fd = -1;
    ...
    l = calloc(1, sizeof(*l));
    FAIL_IF(NULL == l, E__QUIC2__ALLOC_FAILED);
    l->addr = *addr;

    psa_crypto_init();

    if (0 != quic2_tls_listener_init(&l->tls_ctx, &l->sign_cert, &l->pkey,
                                      l->key_ex, l->ciphers,
                                      &l->cert_der, &l->cert_der_len,
                                      l->cert_iov)) {
        LOG_ERROR("QUIC2: TLS listener init failed");
        free(l);
        FAIL(E__QUIC2__INIT_FAILED);
    }

    fd = quic2_udp_bind(addr->ip, addr->port);
    if (fd < 0) {
        LOG_ERROR("QUIC2: bind %s:%d failed: %s", addr->ip, addr->port,
                  strerror(errno));
    ...
    quic2_set_nonblocking(fd);

    l->listen_fd   = fd;
    *out_listen_fd = fd;
    *out_listener  = (skin_listener_t *)l;

    LOG_INFO("QUIC2: listener on %s:%d (udp)", addr->ip, addr->port);
...
static err_t quic2_listener_accept(skin_listener_t *sl,
                                    transport_t **out_transport) {
    err_t rc = E__SUCCESS;
    skin_quic2_listener_t *l = (skin_quic2_listener_t *)sl;
    skin_quic2_ctx_t *ctx    = NULL;
    transport_t *t           = NULL;
    ...
    *out_transport = NULL;
    ...
    LOG_INFO("QUIC2: listener received %zd bytes", n);

    /* Require QUIC long-header (high bit set) */
    if ((pkt[0] & 0x80) == 0) {
        LOG_WARN("QUIC2: ignoring non-long-header packet");
    ...
    int conn_fd = quic2_udp_bind(l->addr.ip, l->addr.port);
    if (conn_fd < 0) {
        LOG_ERROR("QUIC2: per-conn socket failed: %s", strerror(errno));
        FAIL(E__NET__SOCKET_CREATE_FAILED);
    }
    if (0 != connect(conn_fd, (struct sockaddr *)&remote_addr, remote_addrlen)) {
        LOG_WARN("QUIC2: connect to remote failed: %s", strerror(errno));
        close(conn_fd);
        FAIL(E__NET__SOCKET_CONNECT_FAILED);
    }
    quic2_set_nonblocking(conn_fd);

    ctx = quic2_alloc_ctx();
    if (NULL == ctx) { close(conn_fd); FAIL(E__QUIC2__ALLOC_FAILED); }
    ...
    ctx->conn_ref.get_conn  = quic2_get_conn;
    ...
    if (NULL == ctx->cptls.ptls) {
        LOG_ERROR("QUIC2: ptls_new failed");
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__INIT_FAILED);
    }
    *ptls_get_data_ptr(ctx->cptls.ptls) = &ctx->conn_ref;
    ...
    if (0 != ngtcp2_crypto_picotls_configure_server_session(&ctx->cptls)) {
        LOG_ERROR("QUIC2: configure_server_session failed");
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__INIT_FAILED);
    }
    ...
    if (0 != rv) {
        LOG_WARN("QUIC2: decode_version_cid: %s", ngtcp2_strerror(rv));
        quic2_free_ctx(ctx);
        FAIL(E__NET__SOCKET_ACCEPT_FAILED);
    }

    ngtcp2_cid scid;
    quic2_random(scid.data, 8);
    scid.datalen = 8;
    ...
    if (0 != rv) {
        LOG_ERROR("QUIC2: conn_server_new: %s", ngtcp2_strerror(rv));
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__INIT_FAILED);
    }
    ...
    /* Feed the Initial packet */
    {
        ...
        if (0 != rv && NGTCP2_ERR_RETRY != rv)
            LOG_WARN("QUIC2: read_pkt (Initial): %s", ngtcp2_strerror(rv));
        quic2_flush_send(ctx);
    }

    /* Synchronous handshake (5 s timeout) */
    rc = quic2_run_handshake(ctx, &local_addr, local_addrlen, 5);
    if (E__SUCCESS != rc) {
        LOG_WARN("QUIC2: server handshake failed");
        quic2_free_ctx(ctx);
        goto l_cleanup;
    }
    ...
    quic2_set_nonblocking(ctx->rx_pipe[0]);
    quic2_set_nonblocking(ctx->rx_pipe[1]);
    quic2_set_nonblocking(ctx->tx_pipe[0]);
    quic2_set_nonblocking(ctx->tx_pipe[1]);

    rc = quic2_start_io_thread(ctx);
    if (E__SUCCESS != rc) { quic2_free_ctx(ctx); goto l_cleanup; }
    ...
    t = quic2_alloc_transport(ctx, 1, remote_ip, remote_port_n,
                               SKIN_UDP_QUIC2__ops());
    if (NULL == t) {
        ctx->io_running = 0;
        pthread_join(ctx->io_thread, NULL);
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__ALLOC_FAILED);
    }

    LOG_INFO("QUIC2: accepted connection from %s:%d", remote_ip, remote_port_n);
    *out_transport = t;
...
static void quic2_listener_destroy(skin_listener_t *sl) {
    skin_quic2_listener_t *l = (skin_quic2_listener_t *)sl;
    if (NULL == l) return;
...
static err_t quic2_connect(const char *ip, int port, int connect_timeout_sec,
                            transport_t **out_transport) {
    err_t rc = E__SUCCESS;
    skin_quic2_ctx_t *ctx = NULL;
    transport_t *t        = NULL;
    ...
    *out_transport = NULL;
    ...
    ctx = quic2_alloc_ctx();
    FAIL_IF(NULL == ctx, E__QUIC2__ALLOC_FAILED);

    ctx->is_server = 0;

    /* UDP socket connected to remote */
    int fd = socket(AF_INET, SOCK_DGRAM, 0);
    if (fd < 0) { quic2_free_ctx(ctx); FAIL(E__NET__SOCKET_CREATE_FAILED); }
    ctx->udp_fd = fd;
    ...
    if (0 == inet_pton(AF_INET, ip, &remote.sin_addr)) {
        ...
        if (NULL == he) { quic2_free_ctx(ctx); FAIL(E__NET__SOCKET_CONNECT_FAILED); }
    ...
    *ptls_get_data_ptr(ctx->cptls.ptls) = &ctx->conn_ref;
    ...
    /* ALPN "ganon-q2" */
    ctx->alpn_list[0] = ptls_iovec_init(QUIC2_ALPN_STR, QUIC2_ALPN_STRLEN);
    ...
    quic2_random(dcid.data, 8); dcid.datalen = 8;
    quic2_random(scid.data, 8); scid.datalen = 8;
    ...
    int rv = ngtcp2_conn_client_new(&ctx->conn, &dcid, &scid, &path,
                                     ...
                                     &settings, &params, NULL, ctx);
    if (0 != rv) {
        LOG_ERROR("QUIC2: conn_client_new: %s", ngtcp2_strerror(rv));
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__INIT_FAILED);
    }
    ...
    if (0 != ngtcp2_crypto_picotls_configure_client_session(&ctx->cptls,
                                                             ctx->conn)) {
        LOG_ERROR("QUIC2: configure_client_session failed");
        quic2_free_ctx(ctx);
        FAIL(E__QUIC2__INIT_FAILED);
    }
    ...
    rc = quic2_run_handshake(ctx, &ctx->local_addr, ctx->local_addrlen,
                              connect_timeout_sec);
    if (E__SUCCESS != rc) {
        LOG_WARN("QUIC2: client handshake to %s:%d failed", ip, port);
        quic2_free_ctx(ctx);
        goto l_cleanup;
    }
     ...
     * extend_max_local_streams_bidi callback). */
    {
        ...
        if (-1 == ctx->stream_id) {
            LOG_WARN("QUIC2: stream not opened after handshake");
            quic2_free_ctx(ctx);
            FAIL(E__NET__SOCKET_CONNECT_FAILED);
        }
    }
    ...
    t = quic2_alloc_transport(ctx, 0, ip, port, SKIN_UDP_QUIC2__ops());
    if (NULL == t) {
        ctx->io_running = 0;
        pthread_join(ctx->io_thread, NULL);
        quic2_free_ctx(ctx);
    ...
    LOG_INFO("QUIC2: connected to %s:%d", ip, port);
    *out_transport = t;
...
static ssize_t quic2_blocking_read(int fd, void *buf, size_t count) {
    size_t total = 0;
    while (total < count) {
...
static ssize_t quic2_blocking_write(int fd, const void *buf, size_t count) {
    size_t total = 0;
    while (total < count) {
...
static err_t quic2_send_msg(transport_t *t, const protocol_msg_t *msg,
                             const uint8_t *data) {
    err_t rc = E__SUCCESS;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)t->skin_ctx;
    ...
    uint8_t *frame = malloc(QUIC2_PIPE_HDR + payload_len);
    FAIL_IF(NULL == frame, E__QUIC2__ALLOC_FAILED);

    uint32_t net_len = htonl((uint32_t)payload_len);
    memcpy(frame, &net_len, 4);

    size_t written = 0;
    rc = PROTOCOL__serialize(msg, data, frame + QUIC2_PIPE_HDR,
                              payload_len, &written);
    if (E__SUCCESS != rc) { free(frame); goto l_cleanup; }

    size_t total = QUIC2_PIPE_HDR + written;
    LOG_DEBUG("QUIC2: send_msg: writing %zu bytes to tx_pipe[1] (msg_type=%u)", total, msg->type);
    ssize_t wr   = quic2_blocking_write(ctx->tx_pipe[1], frame, total);
    free(frame);

    if (wr != (ssize_t)total) {
        LOG_WARN("QUIC2: send_msg: short write %zd != %zu", wr, total);
        FAIL(E__NET__SOCKET_CONNECT_FAILED);
    }
...
static err_t quic2_recv_msg(transport_t *t, protocol_msg_t *msg,
                             uint8_t **data) {
    err_t rc = E__SUCCESS;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)t->skin_ctx;
    uint8_t hdr[QUIC2_PIPE_HDR];
    uint8_t *frame = NULL;

    VALIDATE_ARGS(t, msg, data);
    *data = NULL;

    ssize_t r = quic2_blocking_read(ctx->rx_pipe[0], hdr, QUIC2_PIPE_HDR);
    if (r != QUIC2_PIPE_HDR) FAIL(E__NET__SOCKET_CONNECT_FAILED);
    ...
    if (frame_len < PROTOCOL_HEADER_SIZE || frame_len > 300000)
        FAIL(E__QUIC2__BAD_FRAME_LEN);

    frame = malloc(frame_len);
    FAIL_IF(NULL == frame, E__QUIC2__ALLOC_FAILED);

    r = quic2_blocking_read(ctx->rx_pipe[0], frame, frame_len);
...
static void quic2_transport_destroy(transport_t *t) {
    if (NULL == t) return;
    skin_quic2_ctx_t *ctx = (skin_quic2_ctx_t *)t->skin_ctx;
    if (NULL == ctx) return;
...
static const skin_ops_t g_udp_quic2_skin = {
    .skin_id           = SKIN_ID__QUIC2,
    .name              = "udp-quic2",
    .listener_create   = quic2_listener_create,
    .listener_accept   = quic2_listener_accept,
    .listener_destroy  = quic2_listener_destroy,
    .connect           = quic2_connect,
    .send_msg          = quic2_send_msg,
    .recv_msg          = quic2_recv_msg,
    .transport_destroy = quic2_transport_destroy,
};

const skin_ops_t *SKIN_UDP_QUIC2__ops(void) {
    return &g_udp_quic2_skin;
}

err_t SKIN_UDP_QUIC2__register(void) {
    return SKIN__register(&g_udp_quic2_skin);
}
