 ...
 * replaces libngtcp2_crypto_picotls.a.
 */
...
#ifndef NGTCP2_CRYPTO_MAX_ENCRYPTION_AES_GCM
...
/* Forward declaration (implemented below; declared in shared.h). */
...
void quic2_crypto_free_conn_data(ngtcp2_conn *conn) {
    ...
    pthread_mutex_unlock(&g_cbuf_lock);
}

/* ── Backend crypto primitives ─────────────────────────────────────────── */

ngtcp2_crypto_aead *ngtcp2_crypto_aead_aes_128_gcm(ngtcp2_crypto_aead *aead) {
  return ngtcp2_crypto_aead_init(aead, (void *)&ptls_mbedtls_aes128gcm);
}

ngtcp2_crypto_md *ngtcp2_crypto_md_sha256(ngtcp2_crypto_md *md) {
  md->native_handle = (void *)&ptls_mbedtls_sha256;
  return md;
}

ngtcp2_crypto_ctx *ngtcp2_crypto_ctx_initial(ngtcp2_crypto_ctx *ctx) {
  ngtcp2_crypto_aead_init(&ctx->aead, (void *)&ptls_mbedtls_aes128gcm);
  ctx->md.native_handle = (void *)&ptls_mbedtls_sha256;
  ctx->hp.native_handle = (void *)&ptls_mbedtls_aes128ctr;
...
crypto_cipher_suite_get_aead_max_encryption(ptls_cipher_suite_t *cs) {
  if (cs->aead == &ptls_mbedtls_aes128gcm ||
      cs->aead == &ptls_mbedtls_aes256gcm)
    return NGTCP2_CRYPTO_MAX_ENCRYPTION_AES_GCM;
  if (cs->aead == &ptls_mbedtls_chacha20poly1305)
...
crypto_cipher_suite_get_hp(ptls_cipher_suite_t *cs) {
  if (cs->aead == &ptls_mbedtls_aes128gcm)   return &ptls_mbedtls_aes128ctr;
  if (cs->aead == &ptls_mbedtls_aes256gcm)   return &ptls_mbedtls_aes256ctr;
  if (cs->aead == &ptls_mbedtls_chacha20poly1305) return &ptls_mbedtls_chacha20;
  return NULL;
}

static int supported_cipher_suite(ptls_cipher_suite_t *cs) {
  return cs->aead == &ptls_mbedtls_aes128gcm ||
         cs->aead == &ptls_mbedtls_aes256gcm ||
         cs->aead == &ptls_mbedtls_chacha20poly1305;
}
...
ngtcp2_crypto_ctx *ngtcp2_crypto_ctx_tls(ngtcp2_crypto_ctx *ctx,
  ...
  if (cs == NULL || !supported_cipher_suite(cs)) return NULL;
  ...
  ctx->max_decryption_failure = crypto_cipher_suite_get_aead_max_decryption_failure(cs);
  return ctx;
}
...
int ngtcp2_crypto_aead_ctx_encrypt_init(ngtcp2_crypto_aead_ctx *aead_ctx,
  ...
  (void)noncelen;
  ptls_aead_context_t *actx = ptls_aead_new_direct(cipher, 1, key, iv);
...
int ngtcp2_crypto_aead_ctx_decrypt_init(ngtcp2_crypto_aead_ctx *aead_ctx,
  ...
  (void)noncelen;
  ptls_aead_context_t *actx = ptls_aead_new_direct(cipher, 0, key, iv);
  if (actx == NULL) return -1;
  aead_ctx->native_handle = actx;
  return 0;
}
...
int ngtcp2_crypto_cipher_ctx_encrypt_init(ngtcp2_crypto_cipher_ctx *cipher_ctx,
                                          ...
                                          const uint8_t *key) {
  ptls_cipher_context_t *actx = ptls_cipher_new(cipher->native_handle, 1, key);
  if (actx == NULL) return -1;
...
int ngtcp2_crypto_hkdf_extract(uint8_t *dest, const ngtcp2_crypto_md *md,
  ...
  ptls_iovec_t saltv = ptls_iovec_init(salt, saltlen);
  ptls_iovec_t ikm   = ptls_iovec_init(secret, secretlen);
  return ptls_hkdf_extract(md->native_handle, dest, saltv, ikm) == 0 ? 0 : -1;
}
...
int ngtcp2_crypto_hkdf_expand(uint8_t *dest, size_t destlen,
  ...
  ptls_iovec_t prk  = ptls_iovec_init(secret, secretlen);
  ptls_iovec_t infv = ptls_iovec_init(info, infolen);
  return ptls_hkdf_expand(md->native_handle, dest, destlen, prk, infv) == 0 ? 0 : -1;
}
...
int ngtcp2_crypto_hkdf(uint8_t *dest, size_t destlen,
  ...
  if (ptls_hkdf_extract(algo, prkbuf, saltv, ikm) != 0) return -1;
  ptls_iovec_t prk = ptls_iovec_init(prkbuf, algo->digest_size);
  return ptls_hkdf_expand(algo, dest, destlen, prk, infv) == 0 ? 0 : -1;
}
...
int ngtcp2_crypto_decrypt(uint8_t *dest, const ngtcp2_crypto_aead *aead,
  ...
  (void)aead;
  ptls_aead_xor_iv(actx, nonce, noncelen);
  size_t nwrite = ptls_aead_decrypt(actx, dest, ciphertext, ciphertextlen,
                                    0, aad, aadlen);
  ptls_aead_xor_iv(actx, nonce, noncelen);
  return nwrite == SIZE_MAX ? -1 : 0;
}
...
int ngtcp2_crypto_read_write_crypto_data(
  ...
  size_t epoch = ngtcp2_crypto_picotls_from_ngtcp2_encryption_level(encryption_level);
  ...
  rv = ptls_handle_message(cptls->ptls, &sendbuf, epoch_offsets, epoch,
                           data, datalen, &cptls->handshake_properties);
  ...
  for (i = 0; i < 4; ++i) {
    ...
    memcpy(stable->buf, sendbuf.base + epoch_offsets[i], epoch_datalen);
    stable->conn = conn;

    pthread_mutex_lock(&g_cbuf_lock);
    stable->next = g_cbuf_head;
    g_cbuf_head  = stable;
    pthread_mutex_unlock(&g_cbuf_lock);

    if (ngtcp2_conn_submit_crypto_data(conn,
          ngtcp2_crypto_picotls_from_epoch(i),
          stable->buf, epoch_datalen) != 0) {
      rv = -1;
      goto fin;
    }
  }
...
/* ── picotls epoch ↔ ngtcp2 level conversions ──────────────────────────── */
...
int ngtcp2_crypto_get_path_challenge_data_cb(ngtcp2_conn *conn, uint8_t *data,
  ...
  ptls_mbedtls_random_bytes(data, NGTCP2_PATH_CHALLENGE_DATALEN);
  return 0;
}

int ngtcp2_crypto_random(uint8_t *data, size_t datalen) {
  ptls_mbedtls_random_bytes(data, datalen);
  return 0;
}

/* ── picotls session configurators ─────────────────────────────────────── */

void ngtcp2_crypto_picotls_ctx_init(ngtcp2_crypto_picotls_ctx *cptls) {
  cptls->ptls = NULL;
  memset(&cptls->handshake_properties, 0,
         sizeof(cptls->handshake_properties));
}
...
static int update_traffic_key_server_cb(ptls_update_traffic_key_t *self,
                                        ptls_t *ptls, int is_enc,
                                        size_t epoch, const void *secret) {
...
static int update_traffic_key_cb(ptls_update_traffic_key_t *self,
                                 ptls_t *ptls, int is_enc,
                                 size_t epoch, const void *secret) {
  ngtcp2_crypto_conn_ref *conn_ref = *ptls_get_data_ptr(ptls);
  ngtcp2_conn *conn = conn_ref->get_conn(conn_ref);
  ...
  if (is_enc)
    return ngtcp2_crypto_derive_and_install_tx_key(conn, NULL, NULL, NULL,
                                                   level, secret, secretlen) == 0 ? 0 : -1;
  return ngtcp2_crypto_derive_and_install_rx_key(conn, NULL, NULL, NULL,
                                                 level, secret, secretlen) == 0 ? 0 : -1;
}
...
int ngtcp2_crypto_picotls_configure_client_session(
  ...
  if (hsprops->client.max_early_data_size == NULL) return -1;
...
void ngtcp2_crypto_picotls_deconfigure_session(
  ...
  if (cptls == NULL) return;
  hsprops = &cptls->handshake_properties;
  free(hsprops->client.max_early_data_size);
  exts = hsprops->additional_extensions;
  if (exts) free(hsprops->additional_extensions[0].data.base);
}
